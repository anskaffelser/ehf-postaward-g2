...
<xsl:stylesheet version="1.0"
	...
	<xsl:variable name="moduleDoc" select="document('/STANDARD/EHFInvoice/1.6/render/Headlines.xml')"/>
	...
	<xsl:template match="cac:PostalAddress | cac:DeliveryAddress | cac:Address | cac:JurisdictionRegionAddress | cac:OriginAddress">
				<div class="UBLPostalAddress">
					...
					<div>
						...
						<span class="UBLCountrySubentity">
...
<!--		
		...
		<xsl:template match="cac:InvoiceLine">
			<tr class="UBLInvoiceLine">
				...
				<td class="UBLPriceUnit">
					<xsl:apply-templates select="cac:Price"/> pr. <xsl:apply-templates select="cac:Price/cbc:BaseQuantity"/>&#160;<xsl:apply-templates select="cac:Price/cbc:BaseQuantity/@unitCode"/>
				</td>
				<td class="UBLTax">
					<xsl:choose>
	...
	<!--Totaler herfra-->
		...
		<xsl:if test="cbc:TaxExclusiveAmount !='' or cbc:AllowanceTotalAmount !='' or cbc:ChargeTotalAmount !='' or cbc:PrepaidAmount !='' or cbc:PayableRoundingAmount !=''">
			<b><xsl:value-of select="$moduleDoc/module/document-merge/g-funcs/g[@name='LegalMonetaryTotal']"/></b>
...
<!--Referencer herfra-->
	...
	<xsl:template match="*">
		<span style="color:red"><xsl:value-of select="local-name()"/>: <xsl:value-of select="."/></span>
	</xsl:template>
