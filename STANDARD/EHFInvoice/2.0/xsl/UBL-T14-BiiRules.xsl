...
<!--PATTERN BIIRULESUBL-T14-->
   ...
   <axsl:template match="//cac:LegalMonetaryTotal" priority="1015" mode="M14">
      ...
      <axsl:choose>
		...
		<!--ASSERT -->
        <!-- 2013-05-13 EG Added check of currency code against DocumentCurrencyCode due to VAT amount in local currency -->
      <axsl:choose>
         <axsl:when test="((cbc:PayableRoundingAmount) and 
                                    (number(cbc:TaxInclusiveAmount) = (round((number(cbc:TaxExclusiveAmount) + 
                                                                                         number(sum(/ubl:CreditNote/cac:TaxTotal/cbc:TaxAmount[@currencyID=/ubl:CreditNote/cbc:DocumentCurrencyCode])) + 
                                                                                         number(cbc:PayableRoundingAmount)) *10 * 10) div 100))) or 
                                                                                         ((number(cbc:TaxInclusiveAmount) = round(( number(cbc:TaxExclusiveAmount) + 
                                                                                                                                               number(sum(/ubl:CreditNote/cac:TaxTotal/cbc:TaxAmount[@currencyID=/ubl:CreditNote/cbc:DocumentCurrencyCode]))) * 10 * 10) div 100) and not (cbc:PayableRoundingAmount))"/>
         <axsl:otherwise>
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl" test="((cbc:PayableRoundingAmount) and 
                                    (number(cbc:TaxInclusiveAmount) = (round((number(cbc:TaxExclusiveAmount) + 
                                                                                         number(sum(/ubl:CreditNote/cac:TaxTotal/cbc:TaxAmount[@currencyID=/ubl:CreditNote/cbc:DocumentCurrencyCode])) + 
                                                                                         number(cbc:PayableRoundingAmount)) *10 * 10) div 100))) or 
                                                                                         ((number(cbc:TaxInclusiveAmount) = round(( number(cbc:TaxExclusiveAmount) + 
   ...
   <axsl:template match="//cac:InvoicePeriod" priority="1006" mode="M14">
      ...
      <axsl:choose>
         <axsl:when test="(cbc:StartDate and cbc:EndDate) and (cbc:StartDate &lt;= cbc:EndDate)"/>
         <axsl:otherwise>
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl" test="(cbc:StartDate and cbc:EndDate) and (cbc:StartDate &lt;= cbc:EndDate)">
               <axsl:attribute name="flag">fatal</axsl:attribute>
               <axsl:attribute name="location">
                  <axsl:apply-templates select="." mode="schematron-get-full-path"/>
               </axsl:attribute>
   ...
   <axsl:template match="/ubl:CreditNote" priority="1003" mode="M14">
      ...
      <axsl:choose>
        ...
        <!-- 2013-05-13 EG Added check of currency code against DocumentCurrencyCode due to VAT amount in local currency -->
      <axsl:choose>
         <axsl:when test="((cac:TaxTotal[cac:TaxSubtotal/cac:TaxCategory/cac:TaxScheme/cbc:ID = 'VAT']/cbc:TaxAmount) and 
        (number(round(sum(cac:TaxTotal//cac:TaxSubtotal/cbc:TaxableAmount[@currencyID=/ubl:CreditNote/cbc:DocumentCurrencyCode]) *10 *10  ) div 100 ) = number(cac:LegalMonetaryTotal/cbc:TaxExclusiveAmount))) or not((cac:TaxTotal[cac:TaxSubtotal/cac:TaxCategory/cac:TaxScheme/cbc:ID = 'VAT']))"/>
         <axsl:otherwise>
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl" test="((cac:TaxTotal[cac:TaxSubtotal/cac:TaxCategory/cac:TaxScheme/cbc:ID = 'VAT']/cbc:TaxAmount) and (number(round(sum(cac:TaxTotal//cac:TaxSubtotal/cbc:TaxableAmount[@currencyID=/ubl:CreditNote/cbc:DocumentCurrencyCode]) *10 *10 ) div 100 ) = number(cac:LegalMonetaryTotal/cbc:TaxExclusiveAmount))) or not((cac:TaxTotal[cac:TaxSubtotal/cac:TaxCategory/cac:TaxScheme/cbc:ID = 'VAT']))">
               <axsl:attribute name="flag">fatal</axsl:attribute>
               <axsl:attribute name="location">
                  <axsl:apply-templates select="." mode="schematron-get-full-path"/>
               </axsl:attribute>
