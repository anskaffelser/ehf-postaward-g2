...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:u="utils"
   ...
   <function xmlns="http://www.w3.org/1999/XSL/Transform" name="u:cat2str">
     <param name="cat"/>
     <value-of select="concat($cat/normalize-space(cbc:ID), '-', round(xs:decimal($cat/cbc:Percent) * 1000000))"/>
   </function>
   ...
   <pattern>
      ...
      <rule context="cac:TaxSubtotal">
         ...
         <assert id="NONAT-T10-R029"
                 ...
                 flag="warning">[NONAT-T10-R029]-Taxable amount in a tax subtotal MUST be the sum of line extension amount of all invoice lines and allowances and charges on document level with the same tax category.</assert>
      </rule>
   </pattern>
</schema>
